...
export default function FoodScannerScreen() {
  ...
  const handleAddToLog = useCallback(async () => {
      ...
      console.log("Attempting to add to log:", recognizedFood);
      try {
          ...
          // This might be different from FoodItem if you add quantity/id here
          const newMealFoods: MealFoodItem[] = recognizedFood.map((food: FoodItem): MealFoodItem => ({
                 id: `food-${Date.now()}-${Math.random().toString(16).slice(2)}`, // Generate unique ID
                 quantity: 100, // Default quantity
                 // --- Map from FoodItem structure ---
                 name: food.name,
                 kcal: Number(food.kcal) || 0, // Ensure saved as number
          ...
          const newMeal: Meal = {
             id: `scan-${Date.now()}`,
             name: recognizedFood[0]?.name || 'Scanned Meal', // Use first item name or default
             time,
             foods: newMealFoods,
          };
  ...
  }, [recognizedFood, navigation, router]); // Dependencies for the callback
      ...
      case 'camera':
       ...
       case 'results':
         ...
         return recognizedFood && recognizedFood.length > 0 ? (
           <ResultsView
             imageUri={capturedImageForDisplay?.uri} // Pass image URI safely
             foodData={recognizedFood}
             onAddToLog={handleAddToLog}         // Pass log handler
             onRetake={handleRetake}             // Pass retake handler for "Scan Again"
           />
         ) : (
            // Render fallback if analysis finished but found nothing
